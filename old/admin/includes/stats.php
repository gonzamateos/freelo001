...
foreach ($pages as $key => $value)
{
	...
	
	{
		if ($key!="")
		{	
			echo htmlspecialchars($value, ENT_QUOTES, 'UTF-8') . " &nbsp; " . "<a target=\"_blank\" class=\"refer\" href='".htmlspecialchars($key, ENT_QUOTES, 'UTF-8')."'>" . substr(htmlspecialchars($key, ENT_QUOTES, 'UTF-8'), 0,45) . "</a><br>\n"; 
			$nb_pages--;
		}
	}
	else break;
...
foreach ($referers as $key => $value)
{
	...
	
	{
		if ($key!=""){
			...
			echo htmlspecialchars($value, ENT_QUOTES, 'UTF-8') . " &nbsp; " . "<a target=\"_blank\" class=\"refer\" href='http://".htmlspecialchars($key, ENT_QUOTES, 'UTF-8')."'>" . substr(htmlspecialchars($key, ENT_QUOTES, 'UTF-8'), 0,45) . "</a><br>\n"; 
			$nb_referers--;
		}
	}
