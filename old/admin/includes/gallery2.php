...
if($fp = fopen($opFile,"r")) {
    ...
    for ($i=0; $i<$no_of_posts; $i++) {
	    ...
	    if($image[1] == $gallery) { 
	        echo '<a href="http://'. $_SERVER['HTTP_HOST'] .'/gudico/'. $pulse_dir .'/data/img/gallery/'. $gallery .'/'. $image[2] .'" rel="lightbox-set" title="'. $image[3] .'" >' . "\n";
            echo '<img  src="http://'. $_SERVER['HTTP_HOST'] .'/gudico/'. $pulse_dir .'/data/img/gallery/'. $gallery .'/'. $image[2] .'" alt="'. $image[3] .'" />';
            echo '</a>' . "\n\n";
	    }
    }
} else {
