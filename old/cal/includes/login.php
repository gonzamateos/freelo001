		...
		if(login_user($user, $password)){
			$url = $phpc_script;
				...
				$url .= '?' . urldecode($vars['lasturl']);
                        ...
                        redirect($url);
			return tag('h2', __('Sesión Iniciada.'));
		}

		$html->add(tag('h2', __('Disculpa, comprueba tus datos de acceso.')));
...
function login_form()
{
        ...
        $submit_data = tag('td', attributes('colspan="2"'),
                                ...
                                create_submit(__('Iniciar Sesión')));
	...
	return tag('form', attributes("action=\"$phpc_script\"",
				'method="post"'),
		tag('table',
			tag('caption', __('Iniciar Sesión')),
                        tag('thead',
                                tag('tr',
                                        tag('th', attributes('colspan="2"'),
                                                __('Debes tener cookies activadas en tu navegador.')))),
			tag('tfoot',
				tag('tr', $submit_data)),
			tag('tbody',
				tag('tr',
