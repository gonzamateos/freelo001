...
function event_delete()
{
	...
	if (empty($vars["confirm"])) {
		...
		$html->add(tag('div', __('¿Seguro que quieres borrar?')));
		$html->add($list);
	...
	if(sizeof($removed_events) > 0) {
		if(sizeof($removed_events) == 1)
			$text = __("Evento borrado");
		else
			$text = __("Eventos borrados");
	...
	if(sizeof($unremoved_events) > 0) {
		if(sizeof($unremoved_events) == 1)
			$text = __("Error al borrar evento");
		else
			$text = __("Error al borrar eventos");
	...
	if(sizeof($permission_denied) > 0) {
		if(sizeof($permission_denied) == 1)
			$text = __("No tienes permisos para borrar este evento");
		else
			$text = __("No tienes permisos para borrar estos eventos");
