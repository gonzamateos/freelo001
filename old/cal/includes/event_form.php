...
function display_form() {
	...
	$form->add_part(new FormFreeQuestion('subject', __('Asunto'),
	...
	if(isset($vars['eid'])) {
		$when_group->add_part(new FormCheckBoxQuestion('phpc-modify',
					false,
					__('Cambia la fecha y hora del evento')));
	}
	...
	$time_type = new FormDropDownQuestion('time-type', __('Tipo de Evento'));
	$time_type->add_option('normal', __('Normal'));
	$time_type->add_option('full', __('Todo el Día'));
	$time_type->add_option('tba', __('Será Anunciado'));

	$when_group->add_part($time_type);

	$form->add_part($when_group);

	$repeat_type = new FormDropdownQuestion('repeats', __('Se repite?'),
			array(), true, 'never');
	...
	$every_day = new FormDropdownQuestion('every-day', __('Cada'),
			__('¿Repetir cada cuántos días?'));
	...
	$every_week = new FormDropdownQuestion('every-week', __('Cada'),
			__('¿Repetir cada cuántas semanas?'));
	...
	$every_month = new FormDropdownQuestion('every-month', __('Cada'),
			__('¿Repetir cada cuántos meses?'));
	...
	$every_year = new FormDropdownQuestion('every-year', __('Cada'),
			__('¿Repetir cada cuántos años?'));
	...
	$categories = new FormDropdownQuestion('catid', __('Categoría'));
	$categories->add_option('', __('Ninguna'));
	...
	$form->add_part(new FormSubmitButton(__("Crear Evento")));
...
function process_form()
{
	...
	if($duration < 0) {
		message(__("La fecha de finalización de un evento no puede ser antes que la fecha de inicio."));
		return display_form();
	}
	}
	...
	if(!$phpc_cal->can_write())
		permission_error(__('No tienes permisos para modificar este calendario.'));
	...
	switch($vars["repeats"]) {
		...
		case 'daily':
			if(!isset($vars["every-day"]))
				soft_error(__("Campo requerido \"every-day\" no esta definido."));
			$ndays = $vars["every-day"];
			if($ndays < 1)
				soft_error(__("cada día debe ser mayor a 1"));
		...
		case 'weekly':
			if(!isset($vars["every-week"]))
				soft_error(__("Campo requerido \"every-week\" no está definido."));
			if($vars["every-week"] < 1)
				soft_error(__("cada semana debe ser mayor a 1"));
		...
		case 'monthly':
			if(!isset($vars["every-month"]))
				soft_error(__("Campo requerido \"every-month\" no está definido."));
			if($vars["every-month"] < 1)
				soft_error(__("cada mes debe ser mayor a 1"));
		...
		case 'yearly':
			if(!isset($vars["every-year"]))
				soft_error(__("Campo requerido \"every-year\" no está definido."));
			if($vars["every-year"] < 1)
				soft_error(__("cada año debe ser mayor a 1"));
	...
	if($eid != 0) {
		...
		return message_redirect(__('Error creando evento.'),
