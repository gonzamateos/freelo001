...
function month_navbar($month, $year) {
	...
	menu_item_append_with_date($nav_tag, __('Año anterior'), 'display_month',
			$year - 1, $month);
	menu_item_append_with_date($nav_tag, __('Mes anterior'), 'display_month',
	...
	menu_item_append_with_date($nav_tag, __('Mes siguiente'), 'display_month',
			$next_year, $next_month);
	menu_item_append_with_date($nav_tag, __('Año siguiente'), 'display_month',
			$year + 1, $month);
...
function create_day($month, $day, $year, $days_events)
{
	...
	foreach($results as $event) {
		if($event == false) {
			$event_html = tag('li',
					create_action_link_with_date(__("Ver eventos adicionales"),
