...
function footer()
{
	...
	$tag = tag('div', attributes('class="phpc-bar ui-widget-content"'),
			"[" . __('Lenguaje') . ": $phpc_lang]" .
			" [" . __('Zona Horaria') . ": $phpc_tz]");
...
function userMenu()
{
	...
	if($action != 'settings')
		menu_item_append($html, __('Configuración'), 'settings');
		
	if(is_user()) {
		menu_item_append($html, __('Cerrar Sesión'), 'logout',
				array('lasturl' =>
					htmlspecialchars(urlencode($_SERVER['QUERY_STRING']))));
	} else {
		menu_item_append($html, __('Iniciar Sesión'), 'login',
				array('lasturl' =>
					htmlspecialchars(urlencode($_SERVER['QUERY_STRING']))));
	}
	return $html;
...
function navbar()
{
	...
	if($action == 'display_event') {
		menu_item_append($html, __('Ver Fecha'), 'display_day', $args);
	}

	if($phpc_cal->can_admin() && $action != 'cadmin') {
		menu_item_append($html, __('Admin del Calendario'), 'cadmin');
	}

	if(is_admin() && $action != 'admin') {
		menu_item_append($html, __('Admin General'), 'admin');
	}
...
function init_config_options() {
	...
	return array( 
			array('week_start', __('Inicio de Semana'), PHPC_DROPDOWN,
				array(
					0 => __('Domingo'),
					1 => __('Lunes'),
					6 => __('Sábado')
				     )),
			array('hours_24', __('Sistema 24 Horas'), PHPC_CHECK),
			array('title', __('Título del Calendario'), PHPC_TEXT),
			array('subject_max', __('Límite de Carácteres del Sujeto'), PHPC_TEXT),
			array('events_max', __('Máximo de Eventos Diarios'), PHPC_TEXT),
			array('anon_permission', __('Permisos Públicos'), PHPC_DROPDOWN,
				array(
					__('No puede ver ni crear eventos'),
					__('Puede leer pero no crear eventos'),
					__('Puede crear pero no modificar eventos'),
					__('Puede crear y modificar eventos')
				     )
			     ),
			array('timezone', __('Zona Horaria'), PHPC_MULTI_DROPDOWN, get_timezone_list()),
			array('language', __('Lenguaje'), PHPC_DROPDOWN,
				$languages),
			array('date_format', __('Formato de Fecha'), PHPC_DROPDOWN,
					get_date_format_list()),
			array('theme', __('Tema'), PHPC_DROPDOWN,
					get_theme_list()),
	);
}
...
function display_phpc() {
	...
	try {
		...
		if($navbar !== false)
			$results->add($navbar);
		$msg = __('No tienes los permisos suficientes para hacer esto: ')
					. $e->getMessage();
...
function get_timestamp($prefix)
{
	...
	if(!isset($vars["$prefix-date"]))
		soft_error(sprintf(__("El campo \"%s\" no fue definido."),
					"$prefix-date"));
	...
	if(!isset($vars["$prefix-time"])) {
		...
		if(!preg_match('/(\d+)[:\.](\d+)\s?(\w+)?/', $vars["$prefix-time"],
					$time_matches)) {
			soft_error(sprintf(__("Error \"%s\" tiempo: \"%s\""),
						$prefix,
						$vars["$prefix-time"]));
		}
		$hour = $time_matches[1];
		...
		if(isset($time_matches[3])) {
			...
			if(strcasecmp("am", $period) == 0) {
				...
				soft_error(__("Periodo no reconocido: ")
						. $period);
			}
		}
	}
	...
	if(!preg_match('/(\d+)[\.\/\-\ ](\d+)[\.\/\-\ ](\d+)/',
		...
		soft_error(sprintf(__("Error \"%s\" fecha: \"%s\""),
