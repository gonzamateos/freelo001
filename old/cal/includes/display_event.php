...
function display_event_by_oid($oid)
{
	...
	$event_header->add(tag('div', __('Fecha de creación: '), $event->ctime));
	if(!empty($event->mtime))
		$event_header->add(tag('div', __('Última modificación: '),
				$event->mtime));
	...
	$menu_tag->add(create_event_link(__('Ver todas las fechas del evento'),
				'display_event', $eid));
...
function display_event_by_eid($eid)
{
	...
	if(!$event->can_read()) {
		return tag('p', __("No tienes permiso para ver este evento."));
	}
	...
	if(!empty($event->mtime))
		$event_header->add(tag('div', ('Última modificación: '),
				$event->mtime));

	$category = $event->get_category();
	if(!empty($category))
		$event_header->add(tag('div', __('Categoría') . ': '
					. $category));
	...
	// Add modify/delete links if this user has access to this event.
		...
		$event_header->add(tag('div', attrs('class="phpc-bar ui-widget-content"'),
					create_event_link(__('Modificar'),
						'event_form', $eid), "\n",
					create_event_link(__('Agregar Fecha al evento'),
						'occur_form', $eid), "\n",
					create_event_link(__('Borrar'),
						'event_delete', $eid)));
	}
	...
	foreach($occurrences as $occurrence) {
		...
		if($event->can_modify()) {
			$occ_tag->add(" ",
					...
					create_occurrence_link(__('Borrar'), 'occurrence_delete', $oid));
		}
	...
	return tag('div', attributes('class="phpc-main phpc-event"'),
			tag('h2', $event->get_subject()), $event_header,
			$desc_tag, tag ('div',attributes('class="phpc-occ"'),tag('h3', __('Fechas')),
			$occurrences_tag));
}
...
function display_event_json()
{
	...
	if(empty($category))
		...
		$category_text = __('Categoría') . ': ' . $event->get_category();

		if ($time_str!="") $time="$date_str " . __("desde") . " $time_str";
		else $time="$date_str ";
